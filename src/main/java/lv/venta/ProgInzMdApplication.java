	...
	public CommandLineRunner testModel(IDriverRepo driverRepo,ICashierRepo cashierRepo, ITripRepo tripRepo, ITicketRepo ticketRepo, ICityRepo cityRepo) {
		return new CommandLineRunner( ) {
			...
			public void run(String... args) throws Exception {
				...
				Trip trip2 = new Trip(new ArrayList<>(Arrays.asList(city3, city4)), dr2, LocalDateTime.now().plusDays(1), 3.0f);
				Trip trip3 = new Trip(new ArrayList<>(Arrays.asList(city1, city3)), dr3, LocalDateTime.now().plusDays(2), 1.5f);
				Trip trip4 = new Trip(new ArrayList<>(Arrays.asList(city2, city4, city5)), dr4, LocalDateTime.now().plusDays(3), 4.0f);
				Trip trip5 = new Trip(new ArrayList<>(Arrays.asList(city1, city3, city5)), dr1, LocalDateTime.now().plusDays(4), 2.0f);
				tripRepo.save(trip1);
				tripRepo.save(trip2);
				tripRepo.save(trip3);
				tripRepo.save(trip4);
				...
				Ticket ticket4 = new Ticket(LocalDateTime.now(), trip4, 9.99f, true, ca2);
				Ticket ticket5 = new Ticket(LocalDateTime.now(), trip5, 15.25f, false, ca1);
